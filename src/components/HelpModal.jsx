...
import {
  ...
  VscSearch, VscDiscard, VscArchive, VscFeedback, VscMortarBoard,
} from 'react-icons/vsc';
import { IoImageOutline, IoColorPaletteOutline, IoCloudOfflineOutline, IoSwapHorizontalOutline } from 'react-icons/io5';
...
const SECTIONS = [
    ...
    content: (
      <>
        ...
        <Steps items={[
          ...
          <>A starter board is created for you on first login.</>,
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <P>Attach images to any task. They're stored alongside the task data (compressed) and sync with your boards.</P>
        <SubH>Upload</SubH>
        <Steps items={[
  ...
  },
  {
    ...
    content: (
      <>
        <H2>Export / Import</H2>
        <P>Back up your boards as files or move them between accounts.</P>
        <SubH>Export</SubH>
        <Steps items={[
    ...
    content: (
      <>
        ...
        <Steps items={[
          ...
          <>When online, changes also sync to your account on the server.</>,
          <>If the network drops, a yellow <strong>Offline</strong> badge appears in the header.</>,
          <>If the server is down but your network is OK, a red <strong>Local only</strong> badge appears.</>,
          <>When you're back online, your changes sync automatically and you'll see a "Reconnected" toast.</>,
        ]} />
        <Tip>Data is keyed per signed-in user, so signing out and signing in as someone else won't leak boards across accounts.</Tip>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        <H2>Send feedback</H2>
        <P>Got a bug, idea, question, or general comment? Click the speech-bubble icon in the header.</P>
        <SubH>Types</SubH>
        <Steps items={[
          <><strong>Bug</strong> — something's broken.</>,
          <><strong>Feature request</strong> — what would you like to see?</>,
          <><strong>Query</strong> — a how-do-I question.</>,
          <><strong>Feedback</strong> — general thoughts.</>,
        ]} />
        <P>All feedback is sent to the maintainer's inbox (and stored in the database for reference). Your email is included automatically so they can reply directly.</P>
      </>
    ),
  },
  {
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
          ...
          >
            ...
            <div style={{ marginTop: '2rem', paddingTop: '1rem', borderTop: '1px solid var(--theme-border)', fontSize: '0.75rem', color: 'var(--theme-text-muted)', textAlign: 'center' }}>
              Have a question this doesn't answer? Click the speech-bubble icon in the header to send feedback.
            </div>
          </div>
        </div>
      </div>
